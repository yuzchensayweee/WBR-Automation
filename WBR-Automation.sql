...
, base15 as (
     select case when {{WMQ}} = 'week'    then date_trunc('week',   order_day+1)::date-1
               ...
               count(distinct case when storage_type = 'F' and lower(sales_region_title) like '%houston%' and split_part(department,'-',1) in ('08','03','04' ) then product_id end) as frozen_instock_tx, -- frozen only 
               ...
               count(distinct case when storage_type = 'F' and lower(sales_region_title) like '%chicago%' and split_part(department,'-',1) in ('08','03','04' ) then product_id end) as frozen_instock_chi,
               ...
               count(distinct case when storage_type = 'F' and lower(sales_region_title) like '%tampa%' and split_part(department,'-',1) in ('08','03','04' ) then product_id end) as frozen_instock_fl,
